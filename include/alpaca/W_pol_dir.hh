 ...
 *
 *
 * Compared to the dir-dir correlation (see the documentation of the W_dir_dir
 * class for more information), it is assumed that both the direction and the
 * polarization of the first photon are known, and that the direction of the
 * second photon is measured.
 *
 * The class uses Eqs. (I-1), (I-1'), (I-2), and (I-4) of Ref. \cite
 * FaggHanna1959 with a similar notation (Note that Ref. \cite FaggHanna1959
 * assumes that the polarization information is given for the exit channel. The
 * case of an incoming polarized photon, which is completely analogous, is shown
 * in Ref. \cite Kneissl1996 and also used here.). It makes use of the dir-dir
 * correlation which is implemented in the class W_dir_dir.
 *
 * The pol-dir correlation is normalized to \f$4 \pi\f$ like the dir-dir
 * correlation.
 */
...
public:
   ...
   * photon and the direction of the outgoing photon in radians.
   *
   ...
   *
   * For the pol-dir correlation, the maximum possible value cannot be
   * calculated analytically, but an upper limit can be given. It is derived
   * here in analogy to the limit for the dir-dir correlation in W_dir_dir.
   ...
   * upper limit is given by Lohöfer {Eq. (22) in \cite Lohoefer1998}:
   *
   * \f[
   * 		\max_{\cos \left( \theta \right) \in \left[ -1, 1 \right]} |
   * P_\nu^{\left( 2 \right)} \left[ \cos \left( \theta \right) \right] | <
   * \frac{4}{\pi^{3/4}} \sqrt{\frac{\left( \nu + 2 \right)!}{\left( \nu-2
   * \right)!}}. \f]
   *
   ...
   * \left( L_2, L_2^\prime, j_3, j_2 \right) | + \sum_\nu | \kappa_\nu \left(
   * L_1, L_1^\prime \right) A_v \left( L_1, L_1^\prime, j_1, j_2 \right) A_v
   * \left( L_2, L_2^\prime, j_3, j_2 \right)| \frac{4}{\pi^{3/4}}
   * \sqrt{\frac{\left( \nu + 2 \right)!}{\left( \nu-2 \right)!}} \f]
   *
   * Note that the first part of the sum is the upper limit for the dir-dir
   * correlation.
   *
...
protected:
   ...
   * correlation.
   *
   * The sum over \f$\nu\f$ in Eqs. (I-1), (I-1'), and (I-8) of \cite
   * FaggHanna1959 contains products of \f$A_\nu\f$, \f$\alpha_\nu\f$ and
   * potentially also \f$U_\nu\f$ coefficients.
   *
   ...
   * 		A_v \left( L_{n-1}, L_{n-1}^\prime, j_n, j_{n-1}, \delta_{n-1}
   * \right). \f]
   *
   * This function calculates all products from \f$\nu = 2\f$ up to, and
   * including, a maximum value of \f$\nu_\mathrm{max}\f$ for a given set of
   * quantum numbers, and returns them as a vector.
   *
   * \return \f$\alpha_\nu (1) A_\nu (n)\f$ for \f$ \nu \in \lbrace 0, ...,
   * \nu_\mathrm{max} \rbrace,~ \nu~\mathrm{even} \f$ sorted by increasing
   * values of \f$\nu\f$ in a std::vector.
  ...
  vector<AlphavCoefficient>
      alphav_coefficients; /**< Vector of AlphavCoefficient objects */
  vector<double>
