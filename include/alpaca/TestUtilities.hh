...
void test_numerical_equality(const T a, const T b, const T epsilon) {
  if (fabs(a - b) > fabs(epsilon)) {
    ...
    error_message << "Test of numerical equality failed. | " << a << " - " << b
                  << " | = " << std::scientific << fabs(a - b) << " > "
                  << epsilon;
 ...
 * numbers are positive, as commonly assumed when applying the triangle
 *inequality.
 *
 * \param j1 \f$j_1\f$
 * \param j2 \f$j_2\f$
 * \param J \f$J\f$
 *
 * \return \f$ \left( \left| j_1 - j_2 \right| <= J \right) \&\& \left(J <= j_1
 *+ j_2 \right)\f$
 */
template <typename T>
