 ...
 *
 * Evaluates the angular correlation between two photons which are emitted from
 * an oriented nuclear state. Without loss of generality, the orientation is
 * assumed to be in the z direction. It may result from a nuclear reaction of
 * the type \f$\left( \gamma, \gamma^\prime \right)\f$, in which a nucleus in an
 * initial state with the total angular momentum quantum number ('spin')
 * \f$j_1\f$ absorbs a photon from a beam that travels in positive z direction
 * to render it in an excited state with the spin \f$j_2\f$, and emits another
 * photon in a transition to a lower-lying state with the spin \f$j_n\f$. An
 * arbitrary number of intermediate states \f$j_i\f$ with labels \f$1 < i < n\f$
 * may be populated in the decay process. It is assumed that only the two
 * photons associated with the transitions \f$j_1 \to j_2\f$ and \f$j_{n-1} \to
 * j_n\f$ are observed. Furthermore, the restriction to a dir-dir correlation
 * implies that only information about the direction of the photons is
 * available, but not about their polarization. In particular, this means that
 * the dir-dir correlation does not depend on the parity quantum number of the
 * involved nuclear states, which translates into an independence of the
 * azimuthal angle of the emitted photon.
 *
 * The class uses Eqs. (I-1), (I-1') and (I-2) of Ref. \cite FaggHanna1959 with
 * a similar notation. Note that Eq. (I-2) already includes the assumption that
 * only two multipoles contribute to any of the two transitions, and that the
 * spin of the intermediate states is known. The process can be denoted as {this
 ...
 * observed, is called a cascade.
 *
 * The dir-dir correlation is normalized to \f$4 \pi\f$ here compared to Ref.
 * \cite FaggHanna1959 {see below Eq. (I-2) therein} by dividing through the
...
public:
   ...
   * \f$L_2^\prime\f$. The symbol \f$P_v\f$ denotes a Legendre polynomial of the
   * degree \f$\nu\f$. It incorporates the dependence on the polar angle
   * \f$\theta\f$, while the dependence on the nuclear properties is encoded in
   * the \f$A_\nu\f$ coefficients.
   *
   * For a cascade with more than 2 steps, the so-called \f$U_\nu\f$
   * coefficients enter the previous equation to take into account the
   * deorientation due to the addition decays {Eq. (I-1') in Ref. \cite
   * FaggHanna1959}:
   *
   * \f[
   ...
   * \brief Return value of the dir-dir correlation at an angle \f$\theta\f$
   *
   * This call operator accepts the angle \f$\varphi\f$ as a second argument,
   * although the direction-direction correlation is independent of the
   * azimuthal angle. Internally, the method calls the single-argument call
   * operator.
   *
   * \param theta Polar angle between the direction of the incoming and
   * the outgoing photon in radians.
   * \param phi Azimuthal angle between the polarization axis of the first
   * photon and the direction of the outgoing photon in radians. Note that this
   * argument has no influence on the result.
   *
   * \return \f$W \left( \theta \right)\f$
   */
   ...
   *
   * For the dir-dir correlation, the maximum possible value cannot be
   * calculated analytically, but an upper limit can be given. In the following,
   * the angular correlation without unobserved intermediate transitions is
   ...
   * \right] | \f] \f[ \leq \sum_\nu | A_v \left( L_1, L_1^\prime, j_1, j_2
   * \right) A_v \left( L_2, L_2^\prime, j_3, j_2 \right) | \mathrm{max}_{\theta
   * \in \left[ 0, \pi \right]} | P_\nu \left[ \cos \left( \theta \right)
   * \right] |. \f]
   *
   ...
   * \f]
   *
   * where the $a_i$ are real coefficients and the \f$f_i\f$ real-valued
   * function of a scalar variable \f$x\f$.
   *
   * For the Legendre polynomials \f$P_\nu\f$, the following inequality holds
   * [See, e.g. Eq. (18.14.1) in \cite DLMF2020, which gives an upper limit for
   * the more general Jacobi
   * polynomials.\
   * The relation between Jacobi and Legendre polynomials is given in Eq.
   * (18.7.9) therein]:
   *
   * \f[
   ...
   *
   * This function returns an \f$\nu_\mathrm{max} / 2 \times \left(n-2\right) f$
   * array of \f$U_\nu\f$ coefficient (UvCoefficient) objects as opposed to
   * W_dir_dir::get_Uv_coefficient_products() and
   * W_dir_dir::calculate_expansion_coefficients_Uv() (protected method), which
   * return lists of the products of the columns as a \f$\nu_\mathrm{max} / 2
   * \times 1\f$ array.
   *
   * \return \f$U_\nu\f$ coefficients, sorted by \f$\nu_\mathrm{max}\f$ (first
  ...
  /**
   * \brief Return products of \f$U_\nu\f$ coefficients to be inserted between
   * the \f$A_\nu\f$/\f$/alpha_\nu\f$ coefficients .
   *
   * As opposed to W_dir_dir::get_Uv_coefficients() (see also the documentation
   * of that method), this function returns products of \f$U_\nu\f$ coefficients
   * at a given value of \f$\nu\f$ for all intermediate cascade steps. These are
   * the values that need to be inserted between the \f$A_\nu\f$ or
   * \f$\alpha_\nu\f$ coefficients in an angular correlation to take into
   * account the unobserved intermediate steps [see, e.g. Eqs. (22) and (23) in
   * Ref. \cite Iliadis2021]. As opposed to
   * W_dir_dir::calculate_expansion_coefficients_Uv(), this function does not
...
protected:
   ...
   * W_dir_dir::calculate_expansion_coefficients().
   *
   * \return \f$U_\nu (2) ... U_\nu (n-1)\f$ for \f$ \nu \in \lbrace 0, ...,
   * \nu_\mathrm{max} \rbrace,~ \nu~\mathrm{even} \f$ sorted by increasing
   * values of \f$\nu\f$ in a std::vector.
   */
   ...
   * of coefficients \f$A_\nu\f$ and \f$U_\nu\f$ is nonzero.
   *
   * This limits the sum over \f$\nu\f$ in the definition of the
   * direction-direction correlation. The respective upper limits are given in
   * the documentation of the AvCoefficient and UvCoefficient classes.
   *
   * This function calls the function W_dir_dir::calculate_two_nu_max_Av(),
   * which calculates the upper limits for the \f$A_\nu\f$ coefficients, and, if
   * there are unobserved intermediate transitions, the function
   * W_dir_dir::calculate_two_nu_max_Uv() to do the same for the \f$U_\nu\f$
   * coefficients. The net limit for \f$\nu\f$ will be the more restrictive of
   * both.
   *
   * Note that the same limit also applies for polarization-direction
   * correlations. Although they make use of more general coefficients than the
   * \f$A_\nu\f$, they still depend on the same F coefficients {compare, e.g.,
   * Eq. (I-2) and (I-9) in \cite FaggHanna1959}.
   *
   ...
   * correlation.
   *
   * The sum over \f$\nu\f$ in Eqs. (I-1) and (I-1') of \cite FaggHanna1959
   * contains products of \f$A_\nu\f$ and potentially also \f$U_\nu\f$
   * coefficients.
   *
   * This function calls the function
   * W_dir_dir::calculate_expansion_coefficients_Av() to calculate the products
   * of \f$A_\nu\f$ coefficients from \f$\nu = 0\f$ up to, and including, a
   * maximum value of \f$\nu_\mathrm{max}\f$ for a given set of quantum numbers.
   * If the cascade contains more than two transitions, the corresponding
   * products of \f$U_\nu\f$ coefficients for the same values of \f$\nu\f$ are
   * calculated by the W_dir_dir::calculate_expansion_coefficients_Av()
   * function. This function merges the output of the two functions.
   *
   * \return \f$A_\nu (1) A_\nu (2)\f$ (\f$n=3\f$) or \f$A_\nu (1) U_\nu (2) ...
   * A_\nu (n)\f$ (\f$n>3\f$) for \f$ \nu \in \lbrace 0, ..., \nu_\mathrm{max}
   * \rbrace,~ \nu~\mathrm{even} \f$ sorted by increasing values of \f$\nu\f$ in
   * a std::vector.
   */
   ...
   *
   * As stated below Eqs. (I-2) and (I-1') in Ref. \cite FaggHanna1959, the
   * \f$A_\nu\f$ and \f$U_\nu\f$ coefficients are normalized to \f$1 +
   * \delta^2\f$, where \f$\delta\f$ is the multipole mixing ratio that they
   * contain.
   *
   * In order to normalize the angular correlations to \f$4 \pi\f$, it must be
   * multiplied by:
   *
   * \f[
   ...
   * \f]
   *
   * where \f$\delta_i\f$ is the multipole mixing ratio of the \f$i\f$-th
   * transition.
   *
  ...
  vector<AvCoefficient>
      av_coefficients_excitation; /**< Vector of AvCoefficient objects for the
                                     excitation */
  vector<AvCoefficient> av_coefficients_decay; /**< Vector of AvCoefficient
                                                  objects for the decays */
  vector<vector<UvCoefficient>>
      uv_coefficients; /**< Vector of the UvCoefficient objects */
