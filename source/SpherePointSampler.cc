...
double SpherePointSampler::segment_length_linear_interpolation(
  ...
  for (unsigned int i = 0; i < n_points - 1; ++i) {
    ...
    for (size_t j = 0; j < 3; ++j) {
      segment_increment_squared +=
          (x_i_plus_one[j] - x_i[j]) * (x_i_plus_one[j] - x_i[j]);
    }
...
double SpherePointSampler::find_c(const unsigned int n, const double epsilon,
                                  const unsigned int max_n_iterations) const {
  ...
  error_message << "No value for c found after " << max_n_iterations
                << " iterations with an initial value of c_0 = " << c_0;

  throw runtime_error(error_message.str());
}

double
SpherePointSampler::find_Theta_j(const unsigned int j, const unsigned int n,
  ...
  if (j == 0 || j > n) {
    ...
    stringstream error_message;
    error_message << "j must be an nonzero positive integer between 1 and n = "
                  << n;
  ...
  stringstream error_message;
  error_message << "No value for Theta_j found after " << max_n_iterations
                << " iterations with an initial value of Theta_j_0 = "
