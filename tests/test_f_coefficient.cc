...
int main() {
  ...
  for (auto f : f_coefficient_values) {
    f_coef = make_unique<FCoefficient>(f.two_nu, f.two_L, f.two_Lp, f.two_jp,
                                       f.two_j);
    // Test whether the numerical value is correct within the given digits of
    // the literature data.
    test_numerical_equality<double>(f_coef->get_value(), f.value, epsilon);
    // Check whether the prediction that a given F coefficient vanishes is
    // correct.
    assert(FCoefficient::is_nonzero(f.two_nu, f.two_L, f.two_Lp, f.two_jp,
                                    f.two_j) == (f_coef->get_value() != 0.));
  }
  ...
  // Test some of the safety functions that prevent alpaca from calling GSL
  // functions with false sets of arguments.
  ...
  // The expression below also violates the conservation of angular momentum and
  // the triangle inequality. However, the magnitudes of J and M are compared
  // first in cg_is_nonzero, so this is truly a test of the condition of
  // interest.
