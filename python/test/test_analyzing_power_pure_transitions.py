...
ax.set_xlabel(r"$A(\theta = 90^\circ)$", fontsize=_fontsize_axis_label)
ax.set_xlim(-_xy_max, _xy_max)
ax.set_ylabel(r"$A(\theta = 45^\circ)$", fontsize=_fontsize_axis_label)
ax.set_ylim(-_xy_max, _xy_max)
for i, ana in enumerate(analyzing_powers):
    ax.plot(
        [ana(0.5 * np.pi)],
        [ana(0.25 * np.pi)],
        parity_symbols[0] if i % 2 == 0 else parity_symbols[1],
        color=spin_colors[i // 2],
        markersize=10,
