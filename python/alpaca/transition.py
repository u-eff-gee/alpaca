...
class Transition:
    ...
    def str(
        self,
        separator=" ",
        secondary_in_parentheses=True,
        always_show_secondary=True,
        em_variable_symbol="σ",
    ):
...
string
    String representation.
        ...
        str_rep = em_str_rep(
            self.em_char, em_variable_symbol=em_variable_symbol
        ) + multipolarity_str_rep(self.two_L)

        if (self.delta == 0 or self.delta == 0.0) and not always_show_secondary:
            return str_rep

        str_repp = em_str_rep(
            self.em_charp, em_variable_symbol=em_variable_symbol
        ) + multipolarity_str_rep(self.two_Lp)

        return (
            str_rep
            + separator
            + ("(" if secondary_in_parentheses else "")
            + str_repp
            + (")" if secondary_in_parentheses else "")
        )
...
string
    TeX code
        ...
        tex_rep = (
            em_str_rep(self.em_char, em_variable_symbol=em_variable_symbol)
            + " "
            + multipolarity_str_rep(self.two_L)
        )
        ...
        tex_repp = (
            em_str_rep(self.em_charp, em_variable_symbol=em_variable_symbol)
            + " "
            + multipolarity_str_rep(self.two_Lp)
        )
