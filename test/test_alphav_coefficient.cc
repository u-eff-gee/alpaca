...
int main() {
  ...
  const string str_rep =
      ...
      KappaCoefficient(4, 4, 4).string_representation() +
      FCoefficient(4, 4, 4, 0, 2).string_representation() + "\\delta^{2}";
  assert(av_coef.string_representation() == str_rep);
  assert(av_coef.string_representation(3) ==
         "(-1)\\times\\left(-0.5\\right)\\times0.707+2\\times\\left(-0."
         "167\\right)\\times0\\times\\delta+0.5\\times0\\times\\delta^{2}");
}
